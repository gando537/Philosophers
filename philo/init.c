...
/*   Updated: 2021/08/06 19:38:32 by mdiallo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_mutexes(t_state *state)
{
	...
	state->fork_ = (pthread_mutex_t *)malloc(sizeof(*(state->fork_)) * state->nb_philo);
	if (!(state->fork_))
		return (1);
	i = 0;
...
int	init(t_state *state, int argc, char **argv)
{
	...
	// if (state->nb_philo < 2 || state->nb_philo > 200 || state->time_to_eat < 60
